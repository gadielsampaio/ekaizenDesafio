...
describe('formulário de criação', () => {
  ...
  it('impede clique repetido e novo submit enquanto aguarda a criação', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Salvando...' }).querySelector('[data-slot="spinner"]')).toBeInTheDocument()
  ...
  it('mantém a ordem de foco dos campos e permite salvar com Enter', async () => {
    ...
    await user.tab()
    expect(screen.getByRole('link', { name: 'Voltar para inspeções' })).toHaveFocus()
    await user.tab()
    expect(screen.getByLabelText('Título')).toHaveFocus()
  ...
  it('confirma descarte mesmo quando somente o checklist foi alterado', async () => {
    ...
    const dialog = screen.getByRole('alertdialog', { name: 'Descartar inspeção?' })
    const keep = within(dialog).getByRole('button', { name: 'Continuar editando' })
    expect(keep).toHaveFocus()
    await user.tab({ shift: true })
    expect(within(dialog).getByRole('button', { name: 'Descartar alterações' })).toHaveFocus()
    await user.tab()
    expect(keep).toHaveFocus()
    ...
    await user.click(within(screen.getByRole('alertdialog', { name: 'Descartar inspeção?' })).getByRole('button', { name: 'Descartar alterações' }))
  ...
  it('mostra observações próprias para Não e oculta a observação ao trocar para Sim', async () => {
    ...
    expect(identificacao.getByLabelText('Sim')).toBeChecked()
    expect(identificacao.getByLabelText('Sim')).toHaveFocus()
    expect(screen.queryByLabelText(`Observação — ${CHECKLIST_PERGUNTAS.identificacao}`)).not.toBeInTheDocument()
    await user.keyboard('{ArrowRight}')
    expect(identificacao.getByLabelText('Não')).toBeChecked()
    expect(identificacao.getByLabelText('Não')).toHaveFocus()
    expect(screen.getByLabelText(`Observação — ${CHECKLIST_PERGUNTAS.identificacao}`)).toHaveValue('Etiqueta está ilegível.')
  })
})
...
describe('envio direto pela criação', () => {
  ...
  it('usa o atraso existente e bloqueia repetição, edição e saída durante o envio', async () => {
    ...
    try {
      ...
      expect(screen.getByRole('button', { name: 'Enviando...' }).querySelector('[data-slot="spinner"]')).toBeInTheDocument()
